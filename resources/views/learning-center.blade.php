<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Learning Center</title>
    <link rel="stylesheet" href="css/styles.css">
    <link rel="shortcut icon" type="image/png" href="../admin/images/logos/inkindo-kotak.png" />
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://unpkg.com/leaflet/dist/leaflet.css" />
    <!-- Google tag (gtag.js) -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-3VJSGM32TZ"></script>
    <script>
        window.dataLayer = window.dataLayer || [];

        function gtag() {
            dataLayer.push(arguments);
        }
        gtag('js', new Date());

        gtag('config', 'G-3VJSGM32TZ');
    </script>
</head>

<body>
    <section class="hero-bg-polos">
        <header>
            <div class="container">
                <div class="logo-nav">
                    <a href="/">
                        <img src="../img/logo/inkindo2.png" alt="INKINDO Jawa Timur Logo" class="logo">
                    </a>
                </div>
                <nav id="nav-menu">
                    <ul>
                        <li><a href="/profil">Profil</a></li>
                        <li><a href="/alur">Alur</a></li>
                        <li><a href="/produk">Layanan</a></li>
                        <li><a href="/berita">Berita</a></li>
                        <li><a href="https://lpse.lkpp.go.id/eproc4/lelang" target="_blank"
                                rel="noopener noreferrer">Info Lelang</a></li>
                        <li><a href="/anggota">Anggota</a></li>
                    </ul>
                </nav>
                <button class="hamburger" id="hamburger-menu">
                    <span class="bar"></span>
                    <span class="bar"></span>
                    <span class="bar"></span>
                </button>

            </div>
        </header>




        <!-- Main Hero Content -->
        <!-- Main Hero Content -->
        <div class="hero-content-2">
            <div class="text-content-2">
                <h1>Learning Center</h1>
                <p>Inkindo menawarkan dua jenis layanan utama: Konstruksi dan Non-Konstruksi. Masing-masing layanan
                    dikelompokkan secara khusus untuk memenuhi kebutuhan Anda, dengan penjelasan detail di bawah ini.
                </p>
            </div>
        </div>
    </section>

    <div class="card-container">
        <a class="no-underline" href="/profil">
            <div class="card">
                <div class="icon">
                    <img src="../img/icon/produk.png" alt="Produk Icon">
                </div>
                <h3>Produk</h3>
            </div>
        </a>
        <a class="no-underline" href="/sejarah">
            <div class="card" data-content="sejarah-perusahaan">
                <div class="icon">
                    <img src="../img/icon/konstruksi.png" alt="Konstruksi Icon">
                </div>
                <h3>Konstruksi</h3>
            </div>
        </a>
        <a class="no-underline" href="/visi-misi">
            <div class="card" data-content="visi-misi">
                <div class="icon">
                    <img src="../img/icon/non-konstruksi.png" alt="Non-Konstruksi Icon">
                </div>
                <h3>Non Konstruksi</h3>
            </div>
        </a>
    </div>

    <section>
        <section class="lc-learning-center">
            <div class="lc-card">
                <div class="lc-card-header">
                    <img src="../img/aplikasi/learning-center.png" alt="INKINDO Logo" class="lc-logo">
                    <div class="lc-info">
                        <h2>Learning Center</h2>
                        <p class="lc-category">Website</p>
                        <p class="lc-description">
                            Aplikasi berbasis web sebagai ruang belajar dengan melibatkan pengguna dan instruktur dari
                            berbagai pihak.
                        </p>
                        <div class="lc-rating">
                            <span class="lc-stars">⭐⭐⭐⭐⭐</span>
                            <span class="lc-rating-count">(20 Pelihat)</span>
                        </div>
                        <a href="">
                            <button class="lc-btn-more">Selengkapnya</button>
                        </a>
                    </div>
                </div>

                <div class="lc-card-body">
                    <h3>Apa Itu Website Learning Center?</h3>
                    <p>Website Pelatihan adalah platform yang disediakan oleh Inkindo Jawa Timur untuk anggota yang
                        ingin mengikuti pelatihan yang telah diselenggarakan. Selain itu, website ini juga dapat
                        digunakan oleh calon narasumber yang berminat untuk berkontribusi sebagai pembicara dalam
                        pelatihan yang diadakan.</p>

                    <h4>Kelebihan Website Learning Center</h4>
                    <ul>
                        <li>Manajemen pengguna baik sebagai pengguna maupun istruktur.</li>
                        <li>Informasi konten pelatihan yang dengan mudah dapat diketahui oleh calon peserta.</li>
                        <li>
                            Interaksi antara peserta dengan instruktur pasca pelatihan.
                        </li>
                        <li>
                            Dukungan kemudahan pra pelatihan oleh admin terkait aspek keuangan dan administrasi
                            kepesertaan sehingga instruktur bisa lebih fokus pada penyiapan materi pelatihan.
                        </li>
                        <li>
                            Sertifikat atas kepesertaan yang ditandatangi secara digitak oleh instruktur dengan kop
                            INKINDO Learning Center.
                        </li>
                        <li>
                            Kemudahan pelacakan sertifikat peserta pasca pelatihan.
                        </li>
                    </ul>
                </div>
            </div>
        </section>

    </section>



    <footer>
        <div class="footer-logo">
            <img src="../img/logo/inkindo2.png" alt="INKINDO Jawa Timur">
            <p class="footer-description">
                Ikatan Nasional Konsultan Indonesia (INKINDO) merupakan Asosiasi Perusahaan Jasa Konsultan di Indonesia
                khususnya di Jawa Timur yang didirikan pada tanggal 20 Juni 1979 di Jakarta.
            </p>
            <div class="social-icons">
                <a href="#"><img src="../img/icon/facebook.png" alt=""></a>
                <a href="#"><img src="../img/icon/instagram.png" alt=""></a>
                <a href="#"><img src="../img/icon/youtube.png" alt=""></a>
            </div>
        </div>

        <div class="footer-links">
            <h3>Link</h3>
            <a href="#">Informasi Publik</a>
        </div>

        <div class="footer-contact">
            <h3>Kontak</h3>
            <p><img src="../img/icon/map.png" alt="">Jl. Rungkut Asri Utara No.VII / 7, Kali Rungkut,
                Surabaya, Jawa
                Timur 60132</p>
            <p><img src="../img/icon/phone.png" alt="">(031) 8702310</p>
            <p><img src="../img/icon/envelope.png" alt="">inkindojatim@blabla.com</p>
            <p><img src="../img/icon/globe.png" alt="">inkindojawatimur.org</p>
        </div>

        <div class="footer-copyright">
            <p><?php echo "© " . date("Y") . " INKINDO Jawa Timur."; ?></p>
        </div>
    </footer>



    <script src="../js/script.js"></script>
    <!-- Bootstrap JS -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js"></script>

</body>

</html>
